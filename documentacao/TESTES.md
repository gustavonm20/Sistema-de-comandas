...
A conversão usa a estrutura original preservada em [banco_dados/legado/estrutura_flask_anterior.sql](../banco_dados/legado/estrutura_flask_anterior.sql). Confere valores e preços históricos, preservação da origem, recusa de destino ocupado/igual, rejeição de estrutura incompatível e reversão em caso de falha. Os dois novos testes do terminal verificam a leitura do JSON antigo sem sobrescrevê-lo e a recusa de chaves duplicadas entre idiomas.
...
A [execução 35215464669](https://github.com/gustavonm20/Sistema-de-comandas/actions/runs/35215464669) confirmou novamente os testes e verificou a documentação publicada: 138 ligações locais/imagens em 22 arquivos Markdown.
...
`-MaskInput` exige PowerShell 7.1+. No Windows PowerShell 5.1, use um usuário de teste e configure a senha em um `.env` local temporário ignorado pelo Git; não a inclua no histórico do terminal. Outra opção é executar a mesma suíte pela rotina. Se o banco de testes já estiver inicializado, pule o inicializador: a suíte faz a limpeza controlada. O prefixo é verificado antes de qualquer limpeza. A estrutura de referência acompanha o repositório, portanto os testes também funcionam em uma cópia rasa.
