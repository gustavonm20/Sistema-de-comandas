...
[Índice](README.md) · [Planejamento](../PLANEJAMENTO.md) · [Quadro existente](https://github.com/users/gustavonm20/projects/2/views/4)
...
As descrições das 20 tarefas ativas foram reconciliadas com código, projeto visual e testes, incluindo #11. A #20 passou de MVP para extensão; #24 foi criada para acesso e formulários. Foram reutilizadas as etiquetas existentes `bug`, `enhancement` e `documentation`. Não há encerramentos novos nem atribuição de responsáveis/prazos.

A integração GitHub disponível permite arquivos, registros de alteração, solicitações de integração e tarefas, mas não expõe edição de Projects, criação de etiquetas/marcos ou configuração do repositório. O acesso ao quadro pelo navegador sem sessão retornou 404; isso **não demonstra que o quadro foi excluído**. Sua configuração e seus itens não puderam ser inspecionados. Portanto, as operações abaixo são **pendentes**, não alterações já aplicadas ao Projects.

Não foram encontrados marcos na consulta pública da revisão. As nove etiquetas padrão foram consultadas; nenhuma etiqueta existente foi apagada. Área e prioridade foram registradas nos corpos das tarefas enquanto os campos nativos não estão disponíveis.

## Política de fluxo

| Situação | Condição para entrar | Condição para sair |
| --- | --- | --- |
| Pendências | Escopo futuro ou decisão/dependência ainda aberta | Critérios e dependências suficientes para iniciar |
| A fazer | Trabalho delimitado e pronto para ser escolhido | Desenvolvimento realmente iniciado |
| Em andamento | Alteração ativa, vinculada a tarefa/ramo | Entrega concreta pronta para revisão |
...
Uma solicitação de integração aberta não encerra automaticamente uma tarefa. Duplicada, cancelada e substituída não significam funcionalidade concluída. A #4 permanece fechada como duplicada da #2, fora das métricas de entrega. solicitações de integração #18, #22, #23 e #25 foram integradas e são histórico concluído, sem tornar o escopo atual das tarefas automaticamente pronto.
...
2. Mapear o campo `Status` para **Pendências, A fazer, Em andamento, Em revisão, Concluído**, reaproveitando opções existentes. Não reinicializar o quadro.
3. Reutilizar ou criar `Prioridade` com **Alta, Normal** e `Área` com **Produto, Projeto visual, Aplicação, Dados, Qualidade**, se esses campos trouxerem filtros úteis. Preferir o campo de marco das tarefas a um campo duplicado.
4. Criar/reutilizar os marcos **Base consolidada**, **Atendimento e caixa**, **Interface e resumos**, **Qualidade e acesso**, **Extensões posteriores**, sem prazo. Atribuir conforme a tabela.
5. Adicionar as tarefas reais abaixo somente se ainda não forem itens. Preencher situação, prioridade e área; conferir a evidência antes de movimentar.
...
| [#5](https://github.com/gustavonm20/Sistema-de-comandas/issues/5) | A fazer | normal | projeto visual | Interface e resumos | `enhancement` |
| [#6](https://github.com/gustavonm20/Sistema-de-comandas/issues/6) | A fazer | normal | projeto visual | Interface e resumos | `enhancement` |
| [#7](https://github.com/gustavonm20/Sistema-de-comandas/issues/7) | A fazer | normal | projeto visual | Interface e resumos | `enhancement` |
| [#8](https://github.com/gustavonm20/Sistema-de-comandas/issues/8) | Pendências | normal | projeto visual | Interface e resumos | `enhancement` |
| [#9](https://github.com/gustavonm20/Sistema-de-comandas/issues/9) | A fazer | normal | projeto visual | Interface e resumos | `enhancement` |
...
| [#15](https://github.com/gustavonm20/Sistema-de-comandas/issues/15) | Pendências | normal | aplicação | Interface e resumos | `enhancement` |
...
| [#20](https://github.com/gustavonm20/Sistema-de-comandas/issues/20) | Pendências | normal | dados | Extensões posteriores | `enhancement` |
| [#21](https://github.com/gustavonm20/Sistema-de-comandas/issues/21) | A fazer | alta | dados | Atendimento e caixa | `bug` |
| [#24](https://github.com/gustavonm20/Sistema-de-comandas/issues/24) | Pendências | alta | aplicação | Qualidade e acesso | `enhancement` |

Não há trabalho marcado como Em andamento só por existir no plano. O código de #3 e #11 está na `main`, mas ainda há revisão manual de ambiente/interface nos seus critérios; as demais questões também têm lacunas relevantes. A solicitação de integração de consolidação não substitui esses itens.
