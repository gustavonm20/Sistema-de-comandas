...
- Preserve `protótipo-inicial.py`, os artefatos históricos versionados, a compatibilidade dos dados existentes e a marca. `legado/terminal-json` é histórico e não é importado pela web.
...
`migrar_banco.py` copia apenas a versão Flask anterior de nove tabelas para um banco novo. Não altera a origem e recusa destino ocupado. Os testes usam `banco_dados/legado/estrutura_flask_anterior.sql` como referência imutável; preserve esse arquivo. Mudanças em nomes persistidos devem ser acompanhadas por conversão explícita e testes de preservação.
