...
Este conversor não cobre a estrutura mais antiga do protótipo nem alterações particulares no banco. [banco_dados/legado/estrutura_inicial.sql](../banco_dados/legado/estrutura_inicial.sql) mantém a estrutura histórica com os nomes traduzidos para estudo; não é o instalador da aplicação. A estrutura Flask anterior usada nos testes está preservada em [banco_dados/legado/estrutura_flask_anterior.sql](../banco_dados/legado/estrutura_flask_anterior.sql). A evolução geral de migrações e outros formatos antigos continua na [tarefa #19](https://github.com/gustavonm20/Sistema-de-comandas/issues/19).
